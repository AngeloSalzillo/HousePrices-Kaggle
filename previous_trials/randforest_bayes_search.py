...
search_spaces = {
    "regression__n_estimators": Integer(1000, 2000),
    "regression__max_depth": [None],  # You can keep a list for categorical
    "regression__min_samples_split": Integer(2, 5),
    "regression__min_samples_leaf": Integer(1, 2),
    "regression__max_features": Real(0.8, 0.9)
}

bayes_search = BayesSearchCV(
    estimator=model,
    search_spaces=search_spaces,
    n_iter=30,   # Number of iterations to explore
    cv=5,
    n_jobs=-1,
...
print("CV error: ", -bayes_search.best_score_)


#model applied on the test set
ids = final_test_set["Id"]
final_predictions = np.expm1(bayes_search.predict(X_test))
